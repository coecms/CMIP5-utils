...
# if modlist empty add to it a regex string indicating model name
       ...
       for line in lines:
           ...
           if line.find(exp) and None not in match:
...
# open output files and write header
    odown=open(fdown, "w")
    odown.write("var, mip_table, model, experiment, ensemble, version\n")
    orep=open(frep, "w")
    orep.write("var, mip_table, model, experiment, ensemble, version\n")
    opub=open(fpub, "w")
    opub.write("var, mip_table, model, experiment\n")
